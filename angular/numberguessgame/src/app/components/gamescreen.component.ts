import {Component} from '@angular/core';
import {GameMechanics} from '../services/gamemechanics.service';
import {Router} from '@angular/router';

@Component({
	selector:"gamescreen",
	templateUrl:"./gamescreen.component.html"
})
export class GameScreen {
	guesses:number;
	currentGuess:number;
	currentLow:number;
	currentHigh:number;
	message:string;
	
	constructor(private _game:GameMechanics, private _router:Router) {
		this.guesses = 0;
		this.currentLow = 0;
		this.currentHigh = 100;
		this.message = "Please enter a number between 1 and 100"
	}

	guess() {
		if(isNaN(this.currentGuess)) {
			this.message = "Please enter a NUMBER!"
			return; 
		}
		if(this.currentGuess > this.currentHigh) {
			this.message = "Guess was higher than current highest. Please try again!";
			return;
		}
		if(this.currentGuess < this.currentLow) {
			this.message = "Guess was lower than current lowest. Please try again!";
			return;			
		}
		let temp = this._game.runGame(this.currentGuess);
		if(temp.type === "low") {
			this.message = "Guess was too low. Current low is:"+this.currentGuess;
			this.guesses = temp.guesses;
			this.currentLow = this.currentGuess;
			this.currentGuess = 0;
			return;
		}
		if(temp.type === "high") {
			this.message = "Guess was too high. Current high is:"+this.currentGuess;
			this.guesses = temp.guesses;
			this.currentHigh = this.currentGuess;
			this.currentGuess = 0;
			return;
		}
		alert("You win in "+temp.guesses+"! Congrats!");
		this.guesses=0;
		this.currentLow = 0;
		this.currentHigh = 100;
		this.message = "Please enter a number between 1 and 100";
		this._router.navigate(["/start"]);
	}
	
}